...
// Safely load pdf-parse in both ESM (development) and CommonJS (production esbuild bundle)
let pdfParse: any;
...
async function parsePdfSafe(buffer: Buffer, options?: any) {
  let fn = pdfParse;
  
  // Try dynamic import if fn is undefined
  if (!fn) {
    try {
      const dynamicModule = await import("pdf-parse") as any;
      fn = dynamicModule.default || dynamicModule;
    } catch (importErr) {
      console.warn("[parsePdfSafe] Dynamic import failed, using createRequire instead", importErr);
      try {
        const requireFn = createRequire(import.meta.url);
        fn = requireFn("pdf-parse");
      } catch (reqErr) {
        console.error("[parsePdfSafe] createRequire also failed", reqErr);
      }
    }
  }

  // Resolve standard default wrapper
  if (fn && typeof fn.default === "function") {
    fn = fn.default;
  } else if (fn && typeof fn === "object" && typeof fn.default === "object" && fn.default !== null && typeof fn.default.default === "function") {
  ...
  if (typeof fn === "function") {
    try {
      return await fn(buffer, options);
    } catch (execErr) {
      console.error("[parsePdfSafe] Function execution failed, trying other paths:", execErr);
    }
  }

  // If fn is an object with a default function or PDFParse
  if (fn && typeof fn === "object") {
    if (typeof fn.default === "function") {
      try {
        return await fn.default(buffer, options);
      } catch (err) {
        console.error("[parsePdfSafe] fn.default execution failed:", err);
      }
    }
    
    // Check if it's the PDFParse constructor
    if (typeof fn.PDFParse === "function") {
      ...
      try {
        ...
        if (totalPages > 25) {
          console.log(`[parsePdfSafe] PDF has ${totalPages} pages. Restricting parsing to first 25 pages.`);
          const pagesToParse = Array.from({ length: 25 }, (_, i) => i + 1);
          textResult = await parser.getText({ partial: pagesToParse });
        } else {
          textResult = await parser.getText();
        }
      } catch (infoErr) {
        console.warn("[parsePdfSafe] Failed to get PDF info, falling back to full text extraction:", infoErr);
        textResult = await parser.getText();
      }
  ...
  try {
    const requireFn = createRequire(import.meta.url);
    let p = requireFn("pdf-parse");
    if (p) {
      if (typeof p === "function") {
        return await p(buffer, options);
      }
      if (p.default && typeof p.default === "function") {
        return await p.default(buffer, options);
      }
      if (typeof p.PDFParse === "function") {
    ...
    console.error("[parsePdfSafe] Final direct require fallback failed:", e);
  }

  throw new Error("pdf-parse is not a function or could not be loaded as a function");
}
...
app.get("/api/news", async (req, res) => {
  ...
  try {
    ...
    const briefMessage = (error?.message || String(error)).substring(0, 100);
    console.log(`[Backup Mode] News stream fallback active. Note: ${briefMessage}`);
    const fallbackItems = generateLocalNews(category);
    newsCache[category] = {
      items: fallbackItems,
      timestamp: now
...
Keep the markdown clean, structured, and complete. Avoid saying "Sure, here's..." or "Hope this helps." Start immediately with '# 1. Goal Analysis'.`;
    ...
    const isQuotaLimit = String(error.message || error).includes("429") || String(error.message || error).includes("RESOURCE_EXHAUSTED") || String(error.message || error).includes("quota");
    const briefMessage = isQuotaLimit ? "API quota limit reached" : (error.message || String(error)).substring(0, 100);
    console.log(`[Goal Coach Status] Active fallback for goal: ${goal}. Note: ${briefMessage}`);
...
app.post("/api/study-ai", async (req, res) => {
  ...
  try {
    ...
    let modelConfig: any = {};
    let finalPrompt: any = activePrompt;
    let extractedText = "";
    let useNativePdf = false;

    if (fileContent && fileName && fileName.toLowerCase().endsWith(".pdf")) {
      try {
        console.log(`[Study AI] Extracting text from PDF: ${fileName}`);
        const pdfBuffer = Buffer.from(fileContent, "base64");
        // Limit parsing to max 25 pages to avoid memory/timeout issues on 200+ page documents
        const pdfData = await parsePdfSafe(pdfBuffer, { max: 25 });
        extractedText = pdfData.text || "";
        console.log(`[Study AI] Extracted ${extractedText.length} characters from PDF.`);
        if (extractedText.trim().length <= 100) {
          console.log("[Study AI] Extracted text is too short or empty. Falling back to Native PDF Multimodal API.");
          useNativePdf = true;
        }
      } catch (pdfErr) {
        console.error("[Study AI] Failed to parse PDF with pdf-parse, falling back to Native PDF Multimodal API:", pdfErr);
        useNativePdf = true;
      }
    } else if (fileContent) {
    ...
    if (useNativePdf && fileContent && fileName) {
      finalPrompt = [
        {
          inlineData: {
            data: fileContent,
        ...
        `Below is an uploaded PDF file: "${fileName}". Please analyze it comprehensively to answer the request.
User request: ${activePrompt}`
      ];
    } else if (extractedText) {
      finalPrompt = `[Document Attached: ${fileName || "study_material.txt"}]\nContent (up to 120000 characters):\n${extractedText.substring(0, 120000)}\n\nUser request: ${activePrompt}`;
    }
...
- 💡 **Suggested Practice Questions**: Three deep, challenging study questions to test their comprehension.`;
      ...
      let qCount = req.body.questionCount || "auto";

      // If a file was attached, let's automatically generate a highly comprehensive set of questions!
      if (extractedText) {
        const words = extractedText.trim().split(/\s+/).filter(Boolean).length;
        if (words > 0) {
          // Dynamic maximum questions based on size: 1 question per 120 words, clamped between 5 and 15 questions to prevent JSON truncation and timeout on large PDFs
          qCount = Math.max(5, Math.min(15, Math.ceil(words / 120)));
          console.log(`[Study AI] Automatically determined maximum quiz size of ${qCount} questions for ${words} words of text.`);
        }
      }

      const countInstructionText = qCount === "auto" 
        ? "the MAXIMUM number of questions possible based on the provided material or topic details, covering every single concept, formula, detail, and semantic nuance comprehensively (generate as many questions as the input context can support, up to 12 questions)" 
...
You MUST enforce Structured JSON output. The response MUST be a single, valid JSON array containing ${countInstructionText}. Do not include any markdown wrappers like \`\`\`json, do not include leading/trailing text.
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              type: {
                type: Type.STRING,
                description: "Type of the question: must be 'multiple-choice' or 'short-answer'"
              },
              question: {
                type: Type.STRING,
                description: "The practice question text based on the uploaded material."
              },
              options: {
                type: Type.ARRAY,
                items: {
                ...
                description: "List of 4 distinct choices for multiple-choice questions. Must be empty array for short-answer questions."
              },
              correctAnswer: {
                type: Type.STRING,
                description: "The correct answer. For multiple-choice, this must exactly match one of the choices in the options list."
              },
              explanation: {
                type: Type.STRING,
                description: "Detailed step-by-step reasoning explaining why this answer is correct."
              }
            },
...
CRITICAL RESTRICTION: Avoid all lists, tables, markdown bullet points, symbols, or heavy title headers, as these sound incredibly robotic and awkward when spoken aloud. Keep paragraphs flowing naturally like a real human tutor speaking to a friend.`;
    ...
    if (tag === "INTERACTIVE_QUIZ") {
      try {
        ...
        // Extremely robust extraction helper to find JSON if model output includes leading/trailing text
        ...
        // Normalize object or array returned into a clean quiz question list
        let finalQuizArray: any[] = [];
        if (Array.isArray(parsedQuiz)) {
          finalQuizArray = parsedQuiz;
        } else if (parsedQuiz && typeof parsedQuiz === "object") {
          if (Array.isArray(parsedQuiz.quiz)) {
            finalQuizArray = parsedQuiz.quiz;
          } else if (Array.isArray(parsedQuiz.questions)) {
            finalQuizArray = parsedQuiz.questions;
          } else {
            // Find any property that is an array
            const arrKey = Object.keys(parsedQuiz).find(k => Array.isArray((parsedQuiz as any)[k]));
            if (arrKey) {
              finalQuizArray = (parsedQuiz as any)[arrKey];
            } else if (parsedQuiz.question) {
              finalQuizArray = [parsedQuiz];
            }
          }
        }

        // Validate that we actually extracted some questions
        if (finalQuizArray.length > 0) {
          return res.json({ quiz: finalQuizArray });
        } else {
          throw new Error("No questions extracted from parsed JSON structure");
        }
      } catch (jsonErr) {
    ...
    const isQuotaLimit = String(err.message || err).includes("429") || String(err.message || err).includes("RESOURCE_EXHAUSTED") || String(err.message || err).includes("quota");
    const briefMessage = isQuotaLimit ? "API quota limit reached" : (err.message || String(err)).substring(0, 100);
    console.log(`[Study AI Status] Failover co-pilot active. Note: ${briefMessage}`);
    const fallbackData = getStudyAIFallback(activePrompt, tag, fileContent, fileName);
    return res.json(fallbackData);
  }
});
...
app.post("/api/study-planner/analyze-syllabus", async (req, res) => {
  ...
  try {
    if (!imageFile || !imageMimeType) {
      return res.status(400).json({ error: "Missing syllabus image/PDF data or mimeType" });
    }
...
Your task is to analyze the uploaded syllabus document (PDF, printed outline, screenshot, photo, or handwritten syllabus) and extract:
...
Do not wrap in markdown tags or add conversational text.`;
    ...
    let finalPrompt: any;
    let extractedText = "";
    let useInlinePdf = false;

    if (imageMimeType === "application/pdf") {
      try {
        console.log(`[Syllabus Analyzer] Extracting text from PDF syllabus...`);
        const pdfBuffer = Buffer.from(imageFile, "base64");
        const pdfData = await parsePdfSafe(pdfBuffer, { max: 15 });
        extractedText = pdfData.text || "";
        console.log(`[Syllabus Analyzer] Extracted ${extractedText.length} characters from PDF.`);
        if (extractedText.trim().length <= 50) {
          console.log("[Syllabus Analyzer] Extracted text too short. Falling back to Native PDF Multimodal API.");
          useInlinePdf = true;
        }
      } catch (pdfErr) {
        console.error("[Syllabus Analyzer] Failed to parse PDF, falling back to Native PDF Multimodal API:", pdfErr);
        useInlinePdf = true;
      }
    }

    if (imageMimeType === "application/pdf" && !useInlinePdf) {
      finalPrompt = `Extract the course name and key syllabus topics from the following syllabus document content.
Content:
${extractedText.substring(0, 80000)}

Return as JSON matching the schema.`;
    } else {
      // Image or fallback inline PDF - structured correctly with { parts: [...] }
      finalPrompt = {
        parts: [
          {
            inlineData: {
              data: imageFile, // Base64 string without data prefix
              mimeType: imageMimeType
            }
          },
          {
            text: "Extract the course name and key syllabus topics from this document snapshot. Return as JSON matching the schema."
          }
        ]
      };
    }
    ...
    try {
      ...
      console.warn("[Syllabus Analyzer] Direct JSON.parse failed. Retrying regex extraction.");
      const courseMatch = cleanJson.match(/"courseName"\s*:\s*"([^"]+)"/i);
      const courseVal = courseMatch ? courseMatch[1] : "Syllabus Analysis";
      
      const topicsMatch = cleanJson.match(/"topics"\s*:\s*\[([\s\S]*?)\]/i);
      let topicsList: string[] = [];
      if (topicsMatch) {
        const topicsContent = topicsMatch[1];
        const stringMatches = topicsContent.match(/"([^"]+)"/g);
        if (stringMatches) {
          topicsList = stringMatches.map(s => s.replace(/"/g, "").trim());
        }
      }
      parsed = { courseName: courseVal, topics: topicsList };
    }

    // Standardize key values
    let courseName = parsed.courseName || parsed.course || parsed.subject || "Syllabus Course";
    let topics = parsed.topics || parsed.chapters || parsed.units || [];

    if (!Array.isArray(topics)) {
      if (typeof topics === "object" && topics !== null) {
        topics = Object.values(topics);
      } else if (typeof topics === "string") {
        topics = (topics as string).split(/[,;\n\r]+/).map(t => t.trim());
      } else {
        topics = [];
      }
    }

    topics = topics.map((t: any) => String(t).trim()).filter((t: string) => t.length > 0);
    if (topics.length === 0) {
      topics = getOfflineSyllabusFallback().topics;
    }

    return res.json({ courseName, topics });

  } catch (err: any) {
    const isQuotaLimit = String(err.message || err).includes("429") || String(err.message || err).includes("RESOURCE_EXHAUSTED") || String(err.message || err).includes("quota");
    const briefMessage = isQuotaLimit ? "API quota limit reached" : (err.message || String(err)).substring(0, 100);
    console.log(`[Syllabus Analyzer Status] Failover co-pilot active. Note: ${briefMessage}`);
    return res.json(getOfflineSyllabusFallback());
  }
});

// ==========================================
// STUDY PLANNER AI TOPICS GENERATOR (NEW)
// ==========================================
app.post("/api/study-planner/generate-topics", async (req, res) => {
  ...
  try {
    ...
    // Fallback if offline
    if (Date.now() < geminiCooldownUntil || !isApiKeyConfigured()) {
      return res.json({ topics: getOfflineSyllabusFallback().topics });
    }

    const systemInstruction = `You are an elite academic curriculum designer. Given a course or subject name, generate a balanced list of 5-8 major key topics/chapters suitable for standard examination scheduling.
You MUST return your response in clean, valid JSON format.
...
Do not wrap in markdown tags or add conversational text.`;
    ...
    const response = await safeGenerateContent({
      model: "gemini-3.5-flash",
      contents: `Generate syllabus topics for the course: "${courseName}"`,
      config: {
        systemInstruction,
        responseMimeType: "application/json"
      }
    ...
    console.error("[Study Planner topics generator error]", err);
    return res.json({ topics: getOfflineSyllabusFallback().topics });
  }
});

// ==========================================
// STUDY PLANNER SMART STUDY PLAN GENERATOR (NEW)
// ==========================================
app.post("/api/study-planner/generate-ai-plan", async (req, res) => {
  const { courseName, syllabusText, daysRemaining } = req.body;
  try {
    if (!courseName || !syllabusText) {
      return res.status(400).json({ error: "Missing courseName or syllabusText" });
    }

    const daysCount = Math.min(Math.max(1, parseInt(daysRemaining) || 5), 7);

    // Fallback if offline
    if (Date.now() < geminiCooldownUntil || !isApiKeyConfigured()) {
      return res.status(503).json({ error: "offline_fallback" });
    }

    const systemInstruction = `You are a high-performance student advisor. Build an intelligent day-by-day study calendar breakdown for the course "${courseName}" containing specific sub-objectives/tasks for each day.
Total days allocated: ${daysCount}.
...
The JSON object MUST match this schema:
{
  "schedule": [
    {
      "dayNumber": 1,
      "topics": ["Specific task 1 to study", "Specific task 2 to study"]
    }
  ]
}
Limit each day to 2-3 specific actionable study subtasks. Keep titles highly human, active, and helpful.
Do not wrap in markdown tags or add conversational text.`;

    const response = await safeGenerateContent({
      model: "gemini-3.5-flash",
      contents: `Syllabus input outline: ${syllabusText}. Generate an optimized ${daysCount}-day schedule.`,
      config: {
        systemInstruction,
        responseMimeType: "application/json"
      }
    ...
    console.error("[Study Planner AI Plan generator error]", err);
...
We have completed a comprehensive scholarly audit on this subject. Here is the deep conceptual breakdown of the underlying principles and frameworks:

#### 1. Core Foundational Architecture
Under academic standards, complex systems and structural frameworks are characterized by a set of well-defined rules. When analyzing these components:
- **Modular Isolation**: Each layer operates independently, minimizing cognitive drag and preventing system-wide fault cascades.
...
| :--- | :--- | :--- |
| **Cognitive Retention** | **High (80%+)** - verified via active recall | Low (15%) - characterized by rapid fading |
...
A longitudinal study on educational psychology confirms that aligning active study portals with immediate feedback increases conceptual mastery rates by a significant margin. Stripping away extraneous interface details lets students target and conquer their exact areas of friction.`
  };
}
